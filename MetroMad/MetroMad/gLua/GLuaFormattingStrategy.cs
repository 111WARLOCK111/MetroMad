/*  GLuaFormattingStrategy.cs - A Formatting Strategy for Gmod lua.
...
namespace MetroMad.gLua
{
    ...
    /// </summary>
    public class GLuaFormattingStrategy : DefaultFormattingStrategy
    {
        ...
        static bool IsInsideDocumentationComment(TextArea textArea, ISegment curLine, int cursorOffset)
        {
            for (int i = curLine.Offset; i < cursorOffset; ++i)
            {
                ...
                if (ch == '"')
                {
                    ...
                    // but I don't now object case where a doc comment is after a stringparams object[] anything
                    return false;
                }
        ...
        */
            ...
            switch (ch)
            {
                ...
                case '\n':
                    ...
                    if (textArea.TextEditorProperties.AutoInsertCurlyBracket)
                    {
                        ...
                        var trimmed = oldLineText.TrimStart();
                        if (trimmed.StartsWith("--[[") || trimmed.StartsWith("/*") || trimmed.StartsWith("[["))
                        {
                            if (NeedCurlyEnd(textArea.Document.TextContent, (trimmed.StartsWith("-") ? trimmed.Substring(0, 4) : trimmed.Substring(0, 2)), (trimmed.StartsWith("/") ? "*/" : "]]")))
                            {
                                textArea.Document.Insert(curLine.Offset + curLine.Length,terminator + (trimmed.StartsWith("/") ? "*/" : "]]"));
                                IndentLine(textArea, lineNr + 1);
                            }
                        }
                        ...
                        {
                            if (NeedCurlyEnd(textArea.Document.TextContent, "function", "end"))
                            {
                                textArea.Document.Insert(curLine.Offset + curLine.Length, terminator + "end");
                                IndentLine(textArea, lineNr + 1);
                            }
                        }
                        ...
                        {
                            if (NeedCurlyEnd(textArea.Document.TextContent, "do", "end"))
                            {
                                textArea.Document.Insert(curLine.Offset + curLine.Length, terminator + "end");
                                IndentLine(textArea, lineNr + 1);
                            }
                        ...
                        {
                            if (NeedCurlyEnd(textArea.Document.TextContent, "then", "end"))
                            {
                                textArea.Document.Insert(curLine.Offset + curLine.Length, terminator + "end");
                                IndentLine(textArea, lineNr + 1);
                            }
        ...
        public override int SearchBracketBackward(IDocument document, int offset, char openBracket, char closingBracket)
        {
            ...
            // I don't see object possibility to parse a C# document backwardsparams object[] anything
            ...
            for (int i = 0; i <= offset; ++i)
            {
                ...
                switch (ch)
                {
                    ...
                    case '/':
                        ...
                        if (!inString && !inChar && i + 1 < document.TextLength)
                        {
                            if (!blockComment && (document.GetCharAt(i + 1) == '/' || (document.GetCharAt(i + 1) == '-' && document.GetCharAt(i + 2) != '[' && document.GetCharAt(i + 3) != '[')))
                            {
                                lineComment = true;
                            }
                            if (!lineComment && (document.GetCharAt(i + 1) == '*' || (document.GetCharAt(i + 1) == '-' && document.GetCharAt(i + 2) == '[' && document.GetCharAt(i + 3) == '[')))
                            {
                                blockComment = true;
                            }
                        }
                    ...
                    case '"':
                        if (!(inChar || lineComment || blockComment))
                        {
                            if (inString && verbatim)
                            {
                                if (i + 1 < document.TextLength && (document.GetCharAt(i + 1) == '"' || document.GetCharAt(i + 1) == '\''))
                                {
                                    ++i; // skip escaped quote
                ...
                switch (ch)
                {
                    ...
                    case '/':
                        if (blockComment)
                        {
                            ...
                            if (document.GetCharAt(offset - 1) == '*' || document.GetCharAt(offset - 1) == ']')
                            {
                                blockComment = false;
                            }
                        }
                        ...
                        {
                            if (!blockComment && (document.GetCharAt(offset + 1) == '/' || (document.GetCharAt(offset + 1) == '-' && document.GetCharAt(offset + 1) != '[' && document.GetCharAt(offset + 1) != '[')))
                            {
                                lineComment = true;
                            }
                            if (!lineComment && (document.GetCharAt(offset + 1) == '*' || (document.GetCharAt(offset + 1) == '-' && document.GetCharAt(offset + 2) == '[' && document.GetCharAt(offset + 3) == '[')))
                            {
                                blockComment = true;
                            }
                        }
                    ...
                    case '"':
                        if (!(inChar || lineComment || blockComment))
                        {
                            if (inString && verbatim)
                            {
                                if (offset + 1 < document.TextLength && (document.GetCharAt(offset + 1) == '"' || document.GetCharAt(offset + 1) == '\''))
                                {
        ...
        static bool NeedCurlyEnd(string text, string start, string end)
        {
            int curlyCounter = 0;
            ...
            for (i = 0; i < text.Length; i++)
            {
                ...
                var word = TextHelper.GetIdentifierAt(Core.form.devmad.Document, i);
                ...
                // Can't use switch statement for non-const values, So we will be using the if-else statement
                /*switch (word)
                {*/
                    if (word == start || word == "function" || word == "do")
                        if (!(inString || inChar || lineComment || blockComment))
                        {
                            ++curlyCounter;
                        }
        ...
        public override int SearchWordForward(IDocument document, int offset, string openBracket, string closingBracket)
        {
            bool inString = false;
            bool inChar = false;
            bool verbatim = false;
            ...
            var ran = new System.Collections.Generic.List<int>();
            ...
            while (offset < document.TextLength)
            {
                ...
                switch (ch)
                {
                    ...
                    case '/':
                        ...
                        {*/
                            Debug.Assert(offset > 0);
                            if ((document.GetCharAt(offset - 1) == '*' && ch == '/') || (document.GetCharAt(offset - 1) == ']' && ch == ']'))
                            {
                                //blockComment = false;
                                return offset;
                            }
                    ...
                    case '\\':
                        ...
                        break;*/
                    default:
                        // This is a crappy idea, but works for now. I'll change it later ;)
                        string doeet = ch.ToString() + document.GetCharAt(offset + 1); // CSharp has do keyword, So we're using doeet
                        string ending = doeet + document.GetCharAt(offset + 2);
                        string then = ending + document.GetCharAt(offset + 3);
                        string function = then + document.GetCharAt(offset + 4) + document.GetCharAt(offset + 5) + document.GetCharAt(offset + 6) + document.GetCharAt(offset + 7);


                        if (doeet == "do" || function == "function" || then == "then")
                        {
                            if (!(inString || inChar || lineComment || blockComment))
                            {
                                ++brackets;
