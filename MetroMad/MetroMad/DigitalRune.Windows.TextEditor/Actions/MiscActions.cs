...
namespace DigitalRune.Windows.TextEditor.Actions
{
  ...
  public class ToggleLineComment : AbstractEditAction
  {
      ...
      public override void Execute(TextArea textArea)
      {
          ...
          if (textArea.Document.HighlightingStrategy.Properties.ContainsKey("LineComment2"))
              comment2 = textArea.Document.HighlightingStrategy.Properties["LineComment2"];
          ...
          if (textArea.SelectionManager.HasSomethingSelected)
          {
              ...
              foreach (ISelection selection in textArea.SelectionManager.Selections)
              {
                  ...
                  if (!ShouldComment(textArea.Document, comment2, selection, selection.StartPosition.Y, selection.EndPosition.Y))
                  {
                      shouldComment2 = false;
                      //break;
                  }
              }
              ...
              foreach (ISelection selection in textArea.SelectionManager.Selections)
              {
                  ...
                  if (shouldComment2)
                      SetCommentAt(textArea.Document, comment2, selection, selection.StartPosition.Y, selection.EndPosition.Y);
                  else
                      RemoveCommentAt(textArea.Document, comment2, selection, selection.StartPosition.Y, selection.EndPosition.Y);
          ...
          else
          {
              ...
              if (ShouldComment(textArea.Document, comment2, null, caretLine, caretLine))
                  SetCommentAt(textArea.Document, comment2, null, caretLine, caretLine);
              else
                  RemoveCommentAt(textArea.Document, comment2, null, caretLine, caretLine);
  ...
  public class ToggleBlockComment : AbstractEditAction
  {
    ...
    public override void Execute(TextArea textArea)
    {
        ...
        string start = "BlockCommentBegin";
        string end = "BlockCommentEnd";
      ...
      if (textArea.Document.HighlightingStrategy.Properties.ContainsKey(start))
        commentStart = textArea.Document.HighlightingStrategy.Properties[start];

      string commentEnd = null;
      if (textArea.Document.HighlightingStrategy.Properties.ContainsKey(end))
        commentEnd = textArea.Document.HighlightingStrategy.Properties[end];
