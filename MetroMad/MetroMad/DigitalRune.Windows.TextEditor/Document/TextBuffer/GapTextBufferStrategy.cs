...
namespace DigitalRune.Windows.TextEditor.TextBuffer
{
  ...
  public class GapTextBufferStrategy : ITextBufferStrategy
  {
    ...
    string GetTextInternal(int offset, int length)
    {
      ...
      {
          /*if (_nexttouch && length > 1)
          {
              _nexttouch = false;
              return new string(buffer, _lastoffset, _lastlength + length + 1);
          }
          if (buffer.Length > end && buffer[end] == '.')
          {
              _nexttouch = true;
              _lastlength = length;
              _lastoffset = offset;
