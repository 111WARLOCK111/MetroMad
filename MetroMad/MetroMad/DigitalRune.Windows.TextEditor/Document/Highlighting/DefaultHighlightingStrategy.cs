  ...
  public class DefaultHighlightingStrategy : IHighlightingStrategyUsingRuleSets
  {
    ...
    public virtual void MarkTokens(IDocument document)
    {
      ...
      while (lineNumber < document.TotalNumberOfLines)
      {
        ...
        foreach (var db in words)
        {
            bool Comment = false;
            bool tComment = false;
            int tindex = 0;
            foreach (var mb in words)
            {
                if ((mb.Word == "--" || mb.Word == "//") && tindex <= index)
                    Comment = true;
                else if ((mb.Word == "--[[" || mb.Word == "/*") && tindex <= index && !Comment)
                {
                    Comment = true;
                    tComment = true;
                }
                else if ((mb.Word == "]]" || mb.Word == "*//*") && tComment && tindex <= index)
                {
                    Comment = false;
                    tComment = false;
                }
            ...
            if (!Comment)
            {
                if (db.Word == ":" && index + 1 < words.Count && index - 1 >= 0 && words[0].Word != "//" && words[0].Word != "--")
                {
                    if (words[index - 1].Word != ":" && !String.IsNullOrWhiteSpace(words[index - 1].Word) && (words[index + 1].Word != "." && words[index + 1].Word != ":"))
                    {
                        if (_activeRuleSet.KeyWords[words[index - 1].Word + words[index].Word + words[index + 1].Word] != null)
                        {
                            words[index - 1].Highlight = words[index].Highlight = words[index + 1].Highlight = (HighlightColor)_activeRuleSet.KeyWords[words[index - 1].Word + words[index].Word + words[index + 1].Word];
                        }
                        else if (db.Word == ":" && _activeRuleSet.KeyWords["*" + words[index].Word + words[index + 1].Word] != null)
                        {
                            /*words[index - 1].Highlight = words[index].Highlight =*//*
                            words[index + 1].Highlight = (HighlightColor)_activeRuleSet.KeyWords["*" + words[index].Word + words[index + 1].Word];
                        }
                    }
                }
            }
    ...
    bool MarkTokensInLine(IDocument document, int lineNumber, ref bool spanChanged)
    {
      ...
      List<TextWord> TWords = new List<TextWord>();
      List<TextWord> AWords = new List<TextWord>();
          ...
          if (!Comment)
          {
              if (db.Word.Contains(":"))
              {
                  ...
                  if (p.Length > 1)
                  {
                      ...
                      {
                          if (_activeRuleSet.KeyWords["*:" + p[1]] != null)
                          {
