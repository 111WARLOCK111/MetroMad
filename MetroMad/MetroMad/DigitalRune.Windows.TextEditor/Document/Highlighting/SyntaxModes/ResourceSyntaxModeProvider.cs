...
namespace DigitalRune.Windows.TextEditor.Highlighting
{
  ...
  public class ResourceSyntaxModeProvider : ISyntaxModeFileProvider
  {
    ...
    {
        return GetSyntaxFile(syntaxMode.FileName);
      Assembly assembly = typeof(SyntaxMode).Assembly;
    ...
    public XmlTextReader GetSyntaxFile(string file)
    {
        if (!File.Exists(file))
        {
            System.Windows.Forms.MessageBox.Show("ERROR: syntax file \"" + file + "\" is missing!", "MetroMad", System.Windows.Forms.MessageBoxButtons.OK);
            System.Windows.Forms.Application.Exit();
            return null;
        }
        return new XmlTextReader(File.OpenRead(file));
    }
